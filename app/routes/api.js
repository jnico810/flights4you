...
module.exports = function(app) {
  ...
  app.get('/api/closestAirport', function(req, res) {
    ...
    request(`http://iatageo.com/getCode/${lat}/${lng}`, (error, response, body) => {
      if (!error && response.statusCode == 200) {
        ...
        res.status(400).send("There was an issue finding your closestAirport!");
      }
    });
	});
  ...
  app.get('/api/flights', (req, res) => {
    ...
    const requestData = _setupRequest(origin, dest, date);
    const url = "https://www.googleapis.com/qpxExpress/v1/trips/search?key=AIzaSyAxpUMSDkSHLUzoubyptGkruLbuYit58cw";

    if (origin && dest && date) {
      request({ url: url, method: "POST", json: requestData}, (err, response, body) => {
        if (err || body.error) {
          res.status(400).send("There was an issue finding your flights!");
        } else if (body.trips.tripOption){
          res.status(200).send(_parseFlights(body));
        } else{
          res.status(400).send("There was an issue finding your flights!");
        }
      });
...
const _setupRequest = (origin, dest, date) => {
  return (
  { request: {
    slice: [
      {
